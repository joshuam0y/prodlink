-- Extend follow-up notification cron to include manual reminders created in-app.
...
begin
  ...
  inserted_new_match as (
    ...
    select
      ...
      c.match_id,
      'new_match_nudge',
      'Your match with ' || c.actor_name,
      'Say hi in Messages when you are ready.',
  ...
  inserted_reply as (
    ...
    select
      ...
      'Open Messages when you are ready to reply.',
      '/matches/' || c.match_id::text,
      jsonb_build_object(
        'reminderKey', c.reminder_key,
        'actorId', c.match_id,
  ...
  manual_candidates as (
    select
      ...
      r.match_id,
      r.remind_at,
      coalesce(p.display_name, 'your match') as actor_name,
      'manual_follow_up:' || r.match_id::text || ':' || r.remind_at::text as reminder_key
    from public.message_follow_up_reminders r
    left join public.profiles p
      on p.id = r.match_id
  ...
  inserted_manual as (
    ...
    select
      ...
      c.match_id,
      'reply_nudge',
      'Reminder: follow up with ' || c.actor_name,
      'You asked for a reminder to continue this conversation.',
      '/matches/' || c.match_id::text,
      jsonb_build_object(
        'reminderKey', c.reminder_key,
        'actorId', c.match_id,
        'remindAt', c.remind_at
      )
  ...
  marked_manual as (
    update public.message_follow_up_reminders r
    set done_at = now()
    where r.id in (select c.id from manual_candidates c)
    returning 1
  )
  select
    (select count(*) from inserted_new_match) +
    (select count(*) from inserted_reply) +
    (select count(*) from inserted_manual)
  into inserted_count;
