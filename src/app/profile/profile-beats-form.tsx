  ...
  function save() {
    ...
    startTransition(async () => {
      try {
        ...
        if (!starAudio) {
          // Cover-only allowed (moodboard / artwork visuals).
          // If the user also didn't provide a cover, the payload will store `null`.
        ...
        for (let i = 0; i < slots.length; i++) {
          ...
          if (!au && !cov) {
            setMessage({
              kind: "err",
              text: `Extra “${t}”: add an audio file/URL or a cover image.`,
            });
            return;
          }
          built.push({
